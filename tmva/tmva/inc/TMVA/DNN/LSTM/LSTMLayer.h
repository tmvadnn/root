...
public:
    ...
    inline Matrix_t & CellBackward(Matrix_t & state_gradients_backward,
                                   ...
                                   Matrix_t & forget_gradient, 
                                   Matrix_t & candidate_gradient,
                                   Matrix_t & output_gradient, 
...
TBasicLSTMLayer<Architecture_t>::TBasicLSTMLayer(size_t batchSize, size_t stateSize, size_t inputSize, size_t timeSteps,
   ...
   : VGeneralLayer<Architecture_t>(batchSize, 1, timeSteps, inputSize, 1, timeSteps, stateSize, 2,
    ...
    fInputBiasGradients(this->GetBiasesAt(0)),
    fWeightsForgetGradients(this->GetWeightGradientsAt(0)),
    fWeightsForgetStateGradients(this->GetWeightGradientsAt(1)),
    fForgetBiasGradients(this->GetBiasesAt(0)),
    fWeightsCandidateGradients(this->GetWeightGradientsAt(0)),
    fWeightsCandidateStateGradients(this->GetWeightGradientsAt(1)),
    fCandidateBiasGradients(this->GetBiasesAt(0)),
    fWeightsOutputGradients(this->GetWeightGradientsAt(0)),
    fWeightsOutputStateGradients(this->GetWeightGradientsAt(1)),
    fOutputBiasGradients(this->GetBiasesAt(0))
{
...
TBasicLSTMLayer<Architecture_t>::TBasicLSTMLayer(const TBasicLSTMLayer &layer)
    : VGeneralLayer<Architecture_t>(layer), 
        ...
        fWeightsForgetStateGradients(this->GetWeightGradientsAt(1)),
        fForgetBiasGradients(this->GetBiasesAt(0)),
        fWeightsCandidateGradients(this->GetWeightGradientAt(0)),
        fWeightsCandidateStateGradients(this->GetWeightGradientsAt(1)),
        fCandidateBiasGradients(this->GetBiasesAt(0)),
        fWeightsOutputGradients(this->GetWeightGradientAt(0)),
        fWeightsOutputStateGradients(this->GetWeightGradientsAt(1)),
        fOutputBiasGradients(this->GetBiasesAt(0))
 {
...
-> void
{
    ...
    // Retreive next cell state and next output state.
    ...
    for (size_t t = 0; t < fTimeSteps; ++t) newCellState.emplace_back(this->GetBatchSize(), fCellSize); // T x B x H
    ...
    for(size_t t = 0; t < fTimeSteps; ++t) {
        ...
        // Each gate value at timestep 't' in a recurrent cell.
        CellForward(inputGateValues[t], forgetGateValues[t], candidateValues[t], outputGateValues[t]);
        
        Architecture_t::Copy(newCellState[t], fCell);
...
-> void
{
    ...
    Tensor_t arr_input_gradients_backward;
    for (size_t t = 0; t < fTimeSteps; ++t) arr_input_gradients_backward.emplace_back(this->GetBatchSize(), this->GetInputSize()); // T x B x D
    
    Tensor_t arr_forget_gradients_backward;
    for (size_t t = 0; t < fTimeSteps; ++t) arr_forget_gradients_backward.emplace_back(this->GetBatchSize(), this->GetInputSize()); // T x B x D
    //Architecture_t::Rearrange(arr_forget_gradients_backward, gradients_backward);
    
    Tensor_t arr_candidate_gradients_backward;
    for (size_t t = 0; t < fTimeSteps; ++t) arr_candidate_gradients_backward.emplace_back(this->GetBatchSize(), this->GetInputSize()); // T x B x D
    //Architecture_t::Rearrange(arr_candidate_gradients_backward, gradients_backward);
    
    Tensor_t arr_output_gradients_backward;
    for (size_t t = 0; t < fTimeSteps; ++t) arr_output_gradients_backward.emplace_back(this->GetBatchSize(), this->GetInputSize()); // T x B x D
    //Architecture_t::Rearrange(arr_output_gradients_backward, gradients_backward);
    ...
    Tensor_t arr_output;
    for (size_t t = 0; t < fTimeSteps; ++t) arr_output.emplace_back(this->GetBatchSize(), fStateSize); // B x H
    ...
    for (size_t t = 0; t < fTimeSteps; ++t) arr_actgradients.emplace_back(this->GetBatchSize(), fCellSize);
    ...
    for (size_t t = fTimeSteps; t > 0; t--) {
        ...
        if (t > 1) {
            ...
            CellBackward(state_gradients_backward, prevStateActivations, arr_activations_backward[t-1],
                         arr_input_gradients_backward[t-1], arr_forget_gradients_backward[t-1],
                         arr_candidate_gradients_backward[t-1], arr_output_gradients_backward[t-1],
...
{   
    ...
    return Architecture_t::LSTMLayerBackward(state_gradients_backward, fWeightsInputGradients, fWeightsForgetGradients, fWeightsCandidateGradients,
                                             ...
                                             fCandidateBiasGradients, fOutputBiasGradients, di, df, dc, dout, precStateActivations, fCell,
                                             ...
                                             fWeightsOutputGateState, input, input_gradient, forget_gradient, candidate_gradient, output_gradient);
}
...
-> void
{
 ...
 //______________________________________________________________________________
// template <typename Architecture_t>
// auto TBasicLSTMLayer<Architecture_t>::debugMatrix(const typename Architecture_t::Matrix_t &A, const std::string name = "matrix")
// -> void
// {
    // std::cout << name << "\n";
